    ...
    public List<Personas> getPersonasByColumn(String column, String query) {
        ...
        switch (column) {
            ...
            case "cedulaidentidad":
                System.out.println(personRepo.findByCedulaidentidadLike(query));
                return personRepo.findByCedulaidentidadLike(query);
            case "codeestado":
                // System.out.println(personRepo.findByNombresLike(query));
                return personRepo.findByCodestadoLike(query);
            case "codtipo":
                // System.out.println(personRepo.findByNombresLike(query));
                return personRepo.findByCodtipoLike(query);
            case "fotografia":
                // System.out.println(personRepo.findByNombresLike(query));
                return personRepo.findByFotografiaLike(query);
            case "telefonocelular":
                // System.out.println(personRepo.findByNombresLike(query));
                return personRepo.findByTelefonocelularLike(query);
